...
class ProjectDataCollector:
    ...
    def __init__(self, config_dir: str = ".claude"):
        ...
        # Load configurations with schema validation and caching
        ...
        self.github_token = os.environ.get(
            self.integrations_config.get("integrations", {})
            .get("github", {})
            .get("config", {})
        ...
        self.calendar_api_key = os.environ.get(
            ...
            .get("calendar", {})
            .get("config", {})
            .get("api_key_env", "CALENDAR_API_KEY")
        )
        ...
        for repo_name in repo_names:
            ...
            try:
                ...
                repo_response = requests.get(
                    f"https://api.github.com/repos/{full_repo_name}", headers=headers
                )
    ...
    def collect_notes_data(self, project_name: str) -> NotesData:
        ...
        if NOTES_AVAILABLE:
            try:
                ...
                search_result = notes_interface.execute_command(
                    "research", {"query": project_name, "output_format": "json"}
                )
                ...
                actions_result = notes_interface.execute_command(
                    "check-actions", {"filter_project": project_name, "output_format": "json"}
                )
                ...
                if search_result.success and search_result.data:
                    for note in search_result.data.get("notes", []):
                        ...
                        if any(
                            keyword in content
                            for keyword in ["decision", "decided", "resolved", "concluded"]
                        ):
                            decisions.append(
                                {
                                    "title": note.get("title", "Untitled"),
    ...
    def collect_project_data(self, project_name: str) -> ProjectData:
        ...
        collection_summary["collection_duration_seconds"] = (
        ...
        collection_summary["success_rate"] = (
            ...
            if collection_summary["sources_attempted"]
            else 0
        )
...
def main():
    ...
    parser.add_argument(
        "--cache-only", action="store_true", help="Use cached data only, no API calls"
    )
